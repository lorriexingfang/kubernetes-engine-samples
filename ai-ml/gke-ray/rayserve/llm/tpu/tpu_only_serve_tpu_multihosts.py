...
# === MONKEY-PATCH: Filter out non-TPU nodes to work around vLLM bug ===
# vLLM's ray_distributed_executor.py accesses node['Resources']['TPU'] 
# without checking if the key exists, causing KeyError on head nodes
_original_ray_nodes = ray.nodes

def _filtered_ray_nodes(*args, **kwargs):
    """Filter out nodes without TPU resources."""
    all_nodes = _original_ray_nodes(*args, **kwargs)
    return [n for n in all_nodes if n.get('Resources', {}).get('TPU')]

ray.nodes = _filtered_ray_nodes
# === END MONKEY-PATCH ===
...
#         pg_resources.append({"CPU": 1, "TPU": 1})

#     # Use SPREAD strategy since the deployment may use more than one TPU node.
#     return VLLMDeployment.options(
#         placement_group_bundles=pg_resources,
#         placement_group_strategy="SPREAD").bind(model_id, num_tpu_chips, get_max_model_len(), get_tokenizer_mode(), get_dtype())
...
def build_app(cli_args: Dict[str, str]) -> serve.Application:
    ...
    # Remove placement_group_bundles - let vLLM handle TPU scheduling
